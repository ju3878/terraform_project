// EIP
resource "aws_eip" "project01-eip" {
  domain     = "vpc"
  depends_on = [ aws_internet_gateway.project01-igw ]  
  lifecycle {
...
resource "aws_nat_gateway" "project01-nat" {
  allocation_id = aws_eip.project01-eip.id
